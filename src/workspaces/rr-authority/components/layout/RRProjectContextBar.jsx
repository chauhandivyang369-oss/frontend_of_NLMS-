    ...
    <div className="bg-white border-b border-slate-200 px-4 py-2.5 shadow-2xs select-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        ...
        <div className="flex items-start md:items-center gap-2.5 min-w-0">
          <div className="w-7 h-7 rounded bg-[#1B365D] text-white flex items-center justify-center shrink-0 font-bold text-xs shadow-xs">
            RR
          </div>

          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-mono font-bold text-xs text-[#1B365D] bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">
            ...
            <div className="text-[11px] text-slate-500 flex items-center gap-3 mt-0.5 font-medium flex-wrap">
              <span className="flex items-center gap-1">
                <Building2 className="w-3 h-3 text-slate-400" />
                {selectedProject.requiringBody}
              </span>
              <span>•</span>
              <span className="flex items-center gap-1">
        ...
        <div className="flex items-center gap-3 shrink-0 self-end md:self-center">
          ...
          <div className="hidden sm:flex items-center gap-4 text-xs font-mono">
            <div className="text-right">
              <div className="text-slate-400 text-[10px]">AFFECTED FAMILIES</div>
              <strong className="text-slate-900 text-xs">{selectedProject.totalAffectedFamilies}</strong>
            </div>
            <div className="w-px h-6 bg-slate-200" />
            <div className="text-right">
              <div className="text-slate-400 text-[10px]">DISPLACED</div>
              <strong className="text-rose-600 text-xs">{selectedProject.displacedFamiliesCount}</strong>
            </div>
            <div className="w-px h-6 bg-slate-200" />
            <div className="text-right">
              <div className="text-slate-400 text-[10px]">R&amp;R BUDGET</div>
              <strong className="text-emerald-700 text-xs">₹{selectedProject.budgetAllocatedCr} Cr</strong>
            </div>
          </div>

          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="flex items-center gap-1 px-2.5 py-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium transition-colors cursor-pointer"
          >
