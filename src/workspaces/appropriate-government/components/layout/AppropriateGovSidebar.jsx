...
export default function AppropriateGovSidebar() {
  ...
  return (
    <>
      ...
      `}>
        {/* Sidebar Header */}
        <div className="p-3 border-b border-slate-800 bg-[#0F1E33] flex items-center justify-between">
          ...
          {menuItems.map(item => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-2.5 rounded-lg transition-all flex items-start gap-3 cursor-pointer group relative ${
                  isActive
                    ? 'bg-[#1B365D] text-white shadow-sm border-l-4 border-[#C5A059]'
              ...
              >
                <div className="mt-0.5 shrink-0">
                  <Icon
                    className={`w-4 h-4 transition-colors ${
                      isActive ? 'text-[#E6CA85]' : 'text-slate-400 group-hover:text-slate-200'
                    }`}
                  />
                </div>
                ...
                <div className="flex-1 min-w-0 pr-1">
                  <div className="flex items-center justify-between">
                    ...
                    {item.badge && (
                      <span
                        className={`text-[9px] font-bold text-white px-1.5 py-0.2 rounded-full font-mono shrink-0 ml-1 ${item.badgeColor}`}
                      >
                        {item.badge}
                      </span>
                    )}
        ...
        <div className="p-3 bg-[#0F1E33] border-t border-slate-800 text-[10px] text-slate-400 space-y-1">
          <div className="flex items-center justify-between font-mono">
            <span>Rules Layer:</span>
            <span className="text-emerald-400 font-semibold">v3.1 Configured</span>
          </div>
